"""Command Line Interface (CLI) for Vetra control plane management."""
...
from vetra.config import load_settings
...
@click.group()
@click.version_option(version=__version__, prog_name="vetra")
def cli():
    """Vetra: Key-Value Cache Control Plane for LLM Inference Engines."""
    pass
...
def start(host: str, port: int, mode: str, vllm_url: str):
    """Start the Vetra Control Plane FastAPI daemon."""
    os.environ["VETRA_EXECUTION_MODE"] = mode
    os.environ["VETRA_VLLM__BASE_URL"] = vllm_url

    console.print(
        Panel.fit(
            f"[bold green]Vetra Control Plane v{__version__}[/bold green]\n"
            ...
            title="Starting Vetra Control Plane",
            border_style="green",
        )
    )

    from vetra.api.server import app
    uvicorn.run(app, host=host, port=port, log_level="info")


@cli.command()
@click.option("--api-url", default="http://localhost:8080/api/v1", help="Vetra API base URL.")
def doctor(api_url: str):
    """Diagnose environment, connectivity, Python, Redis, and vLLM status."""
    console.print(f"[bold blue]Vetra Doctor v{__version__}[/bold blue]\n")
    ...
    try:
        ...
        if resp.status_code == 200:
            table.add_row("Vetra API", "[green]ONLINE[/green]", f"Responding at {api_url}")
        else:
            table.add_row("Vetra API", "[yellow]DEGRADED[/yellow]", f"HTTP {resp.status_code}")
    except Exception:
        table.add_row("Vetra API", "[yellow]OFFLINE[/yellow]", "Start with 'vetra start'")
...
def stats(api_url: str):
    ...
    try:
        ...
        table = Table(title="Vetra Live Telemetry Overview")
        ...
        console.print(f"[red]Could not connect to Vetra API at {api_url}: {e}[/red]")
...
def recommendations(api_url: str):
    ...
    try:
        ...
        table = Table(title=f"Vetra Active Recommendations (Total: {data.get('total', 0)}, GPU Pressure: {data.get('gpu_pressure', 0.0):.1%})")
...
def benchmark(workload: str, requests: int):
    """Run baseline vs Vetra benchmark evaluation."""
    from benchmarks.runners.vetra import run_vetra_benchmark
    console.print(f"[bold green]Running Vetra Benchmark:[/bold green] Workload={workload}, Requests={requests}")
    asyncio.run(run_vetra_benchmark(workload_type=workload, num_requests=requests))
...
def config():
    ...
    settings = load_settings()
    import yaml
    console.print(Panel(yaml.dump(settings.model_dump()), title="Vetra Effective Configuration", border_style="blue"))
